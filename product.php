...
<head>
  ...
  ?>
  <title>Roti Sri Bakery — <?php echo htmlspecialchars($name) ?></title>
</head>
<body>
  ...
  <?php
    if (isset($_GET["item_id"])) {
      ...
      if (isset($_GET["buy_now"])) {
        ...
        $cartQty = isset($_GET["qty"]) && $_GET["qty"] > 0 ? $_GET["qty"] : 1;
        ...
        <div class="row">
          <div class="col s4">  
            <a class="magnifier-thumb-wrapper demo">
              <img id="thumb" style="max-height: 350px; max-width: 300px; margin-top: 30px" src="product_images/<?php echo($image); ?>"
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    // Automatically ensure that "Buy Now" has a valid quantity of at least 1
    $('form').submit(function(e) {
        if ($("#qty").val() <= 0) {
            e.preventDefault(); // Prevent form submission
            alert("Please select a quantity of at least 1.");
            $("#qty").val(1); // Default to 1 if no quantity is provided
            $("#sync-qty").val(1); // Sync with hidden input
        }
    });
  });
