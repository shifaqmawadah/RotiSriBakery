...
<head>
  ...
  <title>Roti Sri Bakery - Payment</title>
  <?php 
    include "header.php"; 
...
<body>
  ...
  <input id="exp_month" type="text" name="exp_month" class="validate white-text" 
         pattern="^(0?[1-9]|1[0-2]|Jan(uary)?|Feb(ruary)?|Mar(ch)?|Apr(il)?|May|Jun(e)?|Jul(y)?|Aug(ust)?|Sep(tember)?|Oct(ober)?|Nov(ember)?|Dec(ember)?)$">
  <label for="exp_month">Exp Month</label>
  <span class="helper-text grey-text" data-error="Invalid Month" data-success="Valid Month"></span>
</div>

<div class="input-field col s4">
  <i class="material-icons prefix">event</i>
  <input id="exp_year" type="number" name="exp_year" class="validate white-text" min="2025">
  <label for="exp_year">Exp Year</label>
  <span class="helper-text grey-text" data-error="Year must be 2025 or later" data-success="Valid Year"></span>
</div>
          ...
          <div class="row">
            ...
            <div class="input-field col s4">
              ...
              <input 
                ...
                oninput="this.value = this.value.replace(/\D/g, '').substring(0, 5)">
...
<?php
  ...
  if (isset($_POST["payment"])) 
  {
    ...
    if (isset($_GET["order_id"]))
    {
      ...
      $sql = "INSERT INTO Payment(OrderID, PaymentDate, MemberID)
        VALUES($orderid, CURRENT_TIME, $memberID)";
      ...
      echo("<script>location.href = 'payment_done.php?member_id=" . $memberID . "';</script>");
      exit();
    }
  }
?>
